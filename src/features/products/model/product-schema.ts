...
const oneOf = <const T extends readonly [string, ...string[]]>(
  values: T,
  message: string,
...
const requiredNumber = (requiredMessage: string) =>
  z.number({
    error: (issue) =>
      issue.input === undefined ? requiredMessage : "Wprowadź poprawną liczbę",
  });
...
export const pricingSchema = z.object({
  netPrice: requiredNumber("Cena netto jest wymagana").positive(
    "Cena musi być większa od 0",
  ),
  grossPrice: requiredNumber("Cena brutto jest wymagana").positive(
    "Cena musi być większa od 0",
  ),
  vatRate: z
    .number()
...
export const availabilitySchema = z
  .object({
    ...
    minCartQty: integer("Podaj minimalną ilość").min(
      1,
      "Minimalna ilość musi wynosić co najmniej 1",
    ),
    maxCartQty: integer("Podaj maksymalną ilość").min(
      1,
      "Maksymalna ilość musi wynosić co najmniej 1",
    ),
  })
  ...
  .superRefine(
    (data, ctx) => {
      ...
      const stock = integer("Podaj ilość na magazynie")
        .nonnegative("Ilość nie może być ujemna")
        .safeParse(data.stock);
      if (!stock.success) {
        ctx.addIssue({
          code: "custom",
          path: ["stock"],
          message: stock.error.issues[0]?.message ?? "Nieprawidłowa ilość",
        });
      }
    },
    { when: () => true },
