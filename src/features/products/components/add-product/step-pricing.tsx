...
const vatOptions = VAT_RATES.map((rate) => ({
  value: String(rate),
  label: `${rate}%`,
}));
...
// Setting the derived field without its listeners – otherwise net ⇄ gross would loop forever.
  ...
  render: function StepPricing({ form }) {
    const recalculate = (source: "net" | "gross") => {
      ...
      if (source === "net") {
        form.setFieldValue(
          "grossPrice",
          netToGross(netPrice, vatRate),
          derivedUpdate,
        );
      } else {
        form.setFieldValue(
          "netPrice",
          grossToNet(grossPrice, vatRate),
          derivedUpdate,
        );
      }
    };
    ...
    return (
      <div className="grid gap-4 sm:grid-cols-2">
        <form.AppField
          name="netPrice"
          listeners={{ onChange: () => recalculate("net") }}
        >
          {(field) => (
            <field.NumberField
              label="Cena netto"
        ...
        <form.AppField
          name="grossPrice"
          listeners={{ onChange: () => recalculate("gross") }}
        >
          {(field) => (
            <field.NumberField label="Cena brutto" placeholder="0.00" decimal />
          )}
        </form.AppField>
        <form.AppField
          name="vatRate"
          listeners={{
            onChange: () => recalculate(form.state.values.priceSource),
          }}
        >
          {(field) => (
            <field.SelectField
        ...
        <form.AppField name="currency">
          {(field) => (
            <field.SelectField label="Waluta" options={currencyOptions} />
          )}
        </form.AppField>
      </div>
    );
